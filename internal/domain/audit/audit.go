...
func (a Action) valid() bool {
	...
	case ActionCreateTenant, ActionSetEndpointPrice, ActionSetBankCredential,
		ActionSuspendTenant, ActionActivateTenant:
		return true
	default:
		return false
	}
...
// Entry is an immutable record of one privileged admin-plane action. Fields are
// unexported and exposed via accessors so an entry cannot be mutated after
// construction (append-only at the type level, mirroring billing.LedgerEntry).
type Entry struct {
	id         string
	operatorID string
	action     Action
