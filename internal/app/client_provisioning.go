...
func (s *ClientProvisioningService) ProvisionClient(ctx context.Context, accountID, name, idemKey string) (*tenant.Tenant, error) {
	...
	// Client provisioning NO LONGER mints a webhook ref (SIN-69584 / B1). The single
	// durable ref is minted later at the in-flow C6 registration convergence (cred + PIX
	// key complete, SIN-69560 / F2), so a fresh client holds ZERO refs until then and
	// POST /v1/clients never exposes a capability secret the Conta does not need — the C6
	// PSP calls the webhook, not the Conta. This removes the double-ref / orphan-ref
	// surface that a mint-here-plus-mint-at-F2 flow left behind (the Verz orphan).
	//
	// Record ONLY the created tenant id (not a secret) so the next replay collapses to
	// the same empresa-cliente without a second create.
