...
func (s *BoletoService) RegisterBoleto(ctx context.Context, in RegisterBoletoInput) (*payment.Payment, ports.BoletoResult, error) {
	...
	res, err := s.boleto.CreateBoleto(ctx, in.TenantID, toBoletoRequest(b, in, p.ID()))
	if err != nil {
...
// newDomainBoleto builds and validates the canonical domain boleto from the input,
// attaching the fixed fine (roteiro 2.c) and discount schedule (grupo 3) when present.
func newDomainBoleto(id string, in RegisterBoletoInput, principal shared.Money, tiers []boleto.DiscountTier) (boleto.Boleto, error) {
	b, err := boleto.New(id, in.TenantID, principal, in.DueDate, in.FineBps, in.MonthlyInterestBps)
	if err != nil {
		return boleto.Boleto{}, err
	}
	if in.FineFixedCents > 0 {
		b, err = b.WithFixedFine(in.FineFixedCents)
		if err != nil {
			return boleto.Boleto{}, err
		}
	}
...
// toBoletoRequest maps a validated domain boleto to the PSP port request.
func toBoletoRequest(b boleto.Boleto, in RegisterBoletoInput, boletoID string) ports.BoletoRequest {
