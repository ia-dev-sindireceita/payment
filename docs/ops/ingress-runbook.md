...
tenant id resolvido, nunca o ref (ver a nota de ingress em
`internal/adapters/http/server.go`, na rota `POST /webhooks/c6/{tenantRef}`, e o
mapa `WebhookRefs` em `internal/platform/config/config.go`). Mas **qualquer
intermediário que registre o path completo** (access log de nginx/Envoy/ALB, CDN,
WAF, tracing/APM, request mirroring) persiste o segredo em texto claro →
disclosure de credencial → impersonação total do webhook daquele tenant
(OWASP A02 Cryptographic Failures / A09 Logging Failures).
...
- Webhook C6 não-assinado / `tenantRef` no path (ADR-0002/F4) — nota de ingress em
  `internal/adapters/http/server.go`; review de origem [SIN-64753](/SIN/issues/SIN-64753),
  gate de go-live [SIN-64773](/SIN/issues/SIN-64773)
