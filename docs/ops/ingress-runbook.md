...
ref É a credencial (ADR-0002/F4). A aplicação já está correta — loga apenas o
tenant id resolvido, nunca o ref (ver `internal/adapters/http/server.go:163-170`
e `config.WebhookRefs`). Mas **qualquer intermediário que registre o path
completo** (access log de nginx/Envoy/ALB, CDN, WAF, tracing/APM, request
mirroring) persiste o segredo em texto claro → disclosure de credencial →
impersonação total do webhook daquele tenant (OWASP A02/A09).
...
- F4 — autenticidade do webhook C6 por-tenant (`tenantRef` opaco no path) — [SIN-64753](/SIN/issues/SIN-64753)
- Ingress note da app (loga só o tenant id resolvido, nunca o ref) — `internal/adapters/http/server.go:163-170`, `config.WebhookRefs`
- Gate de go-live do mascaramento de path + rotação/revogação — [SIN-64773](/SIN/issues/SIN-64773) / [SIN-64774](/SIN/issues/SIN-64774)
