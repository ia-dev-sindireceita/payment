...
- **PR-A (esta entrega):** domínio (multa fixa + desconto), `POST /v1/boletos`
  (grupos 1–3) e `GET /v1/boletos/{id}` (grupo 6).
- **PR-B (a seguir):** `DELETE /v1/boletos/{id}` (baixa/cancelamento, grupo 4) e
  `PUT /v1/boletos/{id}` (alteração, grupo 5) + adapter `CancelBoleto`/`UpdateBoleto`.

## Endpoints (multi-tenant, deny-by-default, idempotency obrigatória no POST)
...
| -------------------------------------------- | ---------------------------------------------------------- | --------- |
| Auth deny-by-default (toda rota)             | `http.TestBoletoHTTPErrors/requires_auth`, `/get_requires_auth` | 401 sem token |
| Idempotency key obrigatória no POST          | `http.TestBoletoHTTPErrors/missing_idempotency_key`        | 400 sem header |
| Idempotência (sem cobrança dupla)            | `app.TestRegisterBoletoIdempotent`, `app.TestRegisterBoletoConcurrentSameKey` | 1 ledger entry; mesmo id em N concorrentes |
| OWASP A01 — isolamento cross-tenant (sem oráculo) | `http.TestBoletoCrossTenantGetIsolation`, `bank.TestStubGetBoletoIsolation` | tenant B → 404 no boleto de A |
| Validação no boundary + anti mass-assignment | `http.TestBoletoHTTPErrors/unknown_field`, `/bad_due_date`, `/fine_over_cap` | 400 |
| Erros como valores, sem vazar status C6      | `c6.TestGetBoletoNotFoundMapping` (404→`ErrNotFound`)      | sentinela de domínio, sem corpo upstream |
